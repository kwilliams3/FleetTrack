...
export const addOrUpdateChauffeur = (req: Request, res: Response) => {
  const db = readDB();
...
export const deleteChauffeur = (req: Request, res: Response) => {
  const db = readDB();
  const { id } = req.params;
...
export const assignVehicle = (req: Request, res: Response) => {
  const db = readDB();
