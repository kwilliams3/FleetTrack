...
import { 
  Vehicle, Chauffeur, Versement, Charge, ActivityLog, AffectationHistory, User, UserRole 
} from "../../src/types";
...
export async function getMssqlPool(): Promise<mssql.ConnectionPool | null> {
  ...
  try {
    ...
    console.log("👉 L'application fonctionne sans base de données SQL Server.");
...
/**
 * Firebase Firestore désactivé - Utilisation de SQL Server uniquement
 */
async function writeDBToFirestore(data: AppDatabase): Promise<void> {
  // Désactivé - utilisation de SQL Server uniquement
  // Cette fonction reste vide pour éviter les erreurs Firebase
  return;
}

/**
 * Global database read orchestrator - SQL Server only
 */
export async function readDB(): Promise<AppDatabase> {
  // Utiliser SQL Server comme base principale
  try {
    const pool = await getMssqlPool();
    if (pool) {
    ...
    console.error("⚠️ SQL Server 2014: Échec de lecture.", err?.message);
  }

  // Si SQL Server n'est pas disponible, retourner la base initiale vide
  console.log("📂 Aucune base de données disponible. Retour de la DB par défaut.");
  return INITIAL_DATABASE;
}

/**
 * Global database write orchestrator - SQL Server only
 */
...
// Importer DocumentInfo depuis les types
