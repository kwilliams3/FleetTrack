...
export default function DashboardView({
  ...
  const handlePrintReport = () => {
    ...
    if (printWindow) {
      printWindow.document.write(`
        <html>
          <head>
            <title>Rapport d'Activite Financiere de la Flotte (5 derniers jours)</title>
            <style>
  ...
  vehicles.forEach(v => {
    if (v.documents.assurance?.statut === "expire") {
      systemAlerts.push({ type: "critical", message: `Assurance expirée sur le véhicule ${v.immatriculation}` });
    } else if (v.documents.assurance?.statut === "expirant") {
      systemAlerts.push({ type: "warning", message: `L'assurance du véhicule ${v.immatriculation} expire bientôt` });
    }
  });
  ...
  const financialHistoryChart = last5Days.map(dayStr => {
    ...
    return {
      name: dayStr.split("-").slice(1).reverse().join("/"),
      Attendu: expected,
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Welcome Banner - Version simplifiée sans SVG */}
              ...
              <div className="p-5 border-b border-slate-100">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white font-semibold text-xs px-4 py-2 rounded-xl flex items-center space-x-2 transition-all duration-300 shadow-md"
                  >
                    ...
                    <div className="h-56 w-full relative">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <div className="text-center">
                          ...
                          <p className="text-sm font-bold text-slate-800">{formatFCFA(expenseChartData.reduce((sum, d) => sum + d.value, 0))}</p>
                        </div>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 mt-4 pt-3 border-t border-slate-100">
                      {expenseChartData.slice(0, 4).map((data, idx) => (
                        <div key={idx} className="flex items-center space-x-2 text-xs">
                          ...
                          <span className="text-slate-600 truncate flex-1">{data.name}</span>
                          <span className="text-slate-800 font-semibold text-[10px]">{formatFCFA(data.value)}</span>
                        </div>
                      ))}
                    </div>
                  </>
              ...
              <table className="w-full">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr className="text-[11px] font-mono text-slate-500 uppercase">
                    ...
                    <th className="px-5 py-3 text-right">Versements</th>
                    <th className="px-5 py-3 text-right">Dépenses</th>
                    <th className="px-5 py-3 text-right">Bénéfice Net</th>
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-slate-50/50 transition-colors duration-150">
                        <td className="px-5 py-3">
                          <div>
                            <span className="font-mono text-xs font-bold text-slate-700">{item.matricule}</span>
                        ...
                        <td className={`px-5 py-3 text-right font-mono text-sm font-bold ${isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
                          {formatFCFA(item.profit)}
                        </td>
                        <td className="px-5 py-3 text-right">
                          <span className={`inline-block px-2 py-1 rounded-lg text-xs font-bold ${isPositive ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white rounded-t-2xl sticky top-0 z-10">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800">Rapport d'Activité Financière</h2>
                  <p className="text-xs text-slate-500 font-mono">Période : 08/06 au 12/06/2026</p>
                </div>
              </div>
              <div className="flex items-center space-x-2">
                <button
                  onClick={handleDownloadPDF}
                  className="bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white font-semibold text-xs px-4 py-2 rounded-xl flex items-center space-x-2"
                >
                ...
                <button
                  ...
                  className="bg-slate-700 hover:bg-slate-800 text-white font-semibold text-xs px-4 py-2 rounded-xl flex items-center space-x-2"
                >
            ...
            <div className="p-8 bg-slate-100 overflow-y-auto flex-1">
              <div id="rapport-financier-print-template" className="bg-white p-8 shadow-xl border border-slate-200 w-full max-w-[21cm] mx-auto">
                <div className="text-center border-b-2 border-slate-800 pb-5 mb-6">
                  <h1 className="text-2xl font-bold uppercase tracking-wider text-slate-900">Rapport d'Activité Financière de la Flotte</h1>
                  <p className="text-sm text-slate-500 mt-2">Généré le 12 Juin 2026 par FleetTrack Management</p>
                  <p className="text-xs font-mono text-slate-400">Période : 08 Juin 2026 - 12 Juin 2026 (5 derniers jours)</p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                  <div className="p-4 rounded-lg border border-slate-200 bg-slate-50">
                    <h3 className="text-xs font-bold text-slate-500 uppercase">Recettes Validées</h3>
                    <p className="text-xl font-bold text-emerald-600 mt-1">{formatFCFA(repTotalCollected)}</p>
                  </div>
                  <div className="p-4 rounded-lg border border-slate-200 bg-slate-50">
                    <h3 className="text-xs font-bold text-slate-500 uppercase">Dépenses Approuvées</h3>
                    <p className="text-xl font-bold text-rose-600 mt-1">{formatFCFA(repTotalSpent)}</p>
                  </div>
                  <div className="p-4 rounded-lg border border-slate-200 bg-emerald-50">
                    <h3 className="text-xs font-bold text-emerald-800 uppercase">Solde Net</h3>
                    <p className="text-xl font-bold text-indigo-700 mt-1">{formatFCFA(repTotalBalance)}</p>
                  </div>
                </div>

                <div className="mt-8 pt-4 border-t border-slate-200 flex justify-between text-xs text-slate-500">
                  <div className="w-48 text-center border-t border-dashed border-slate-300 pt-2">
